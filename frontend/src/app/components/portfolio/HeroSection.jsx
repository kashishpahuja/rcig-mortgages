...
export default function HeroSection({ heroData = null }) {
  ...
  useEffect(() => {
    // If props weren't passed down from server, fetch from API directly
    if (!heroData) {
      fetch('/api/hero')
        ...
        .catch(err => console.error("Failed to load hero text:", err));
    }
  }, [heroData]);
     ...
     <Magnet
        ...
        <FadeIn delay={0.25} y={10}>
          ...
          <div className="w-full flex justify-center sm:justify-start mt-2">
            ...
            <div className="h-1 w-[100px] sm:w-[150px] md:w-[190px] bg-[#B31313] rounded-full" />
      ...
      {/* Bottom Gold Border */}
      <div className="absolute bottom-0 left-0 right-0 h-0.75 bg-[#D4AF37]/80 z-30" />
